...
# # WE CHOOSE TO NOT RESTRICT (IN ORDER TO HAVE THE RANKS TO INCLUDE ALL THE GENES SELECTED OR NOT)
...
# lets create the 10 cols for each of the 10 seeds ranking
fs_table_raw$Rank_coef_seed_0[order(-fs_table_raw$Abs_val_coef_seed_0)] <- 1:nrow(fs_table_raw)
fs_table_raw$Rank_coef_seed_1[order(-fs_table_raw$Abs_val_coef_seed_1)] <- 1:nrow(fs_table_raw)
fs_table_raw$Rank_coef_seed_2[order(-fs_table_raw$Abs_val_coef_seed_2)] <- 1:nrow(fs_table_raw)
fs_table_raw$Rank_coef_seed_3[order(-fs_table_raw$Abs_val_coef_seed_3)] <- 1:nrow(fs_table_raw)
fs_table_raw$Rank_coef_seed_4[order(-fs_table_raw$Abs_val_coef_seed_4)] <- 1:nrow(fs_table_raw)
fs_table_raw$Rank_coef_seed_5[order(-fs_table_raw$Abs_val_coef_seed_5)] <- 1:nrow(fs_table_raw)
fs_table_raw$Rank_coef_seed_6[order(-fs_table_raw$Abs_val_coef_seed_6)] <- 1:nrow(fs_table_raw)
fs_table_raw$Rank_coef_seed_7[order(-fs_table_raw$Abs_val_coef_seed_7)] <- 1:nrow(fs_table_raw)
fs_table_raw$Rank_coef_seed_8[order(-fs_table_raw$Abs_val_coef_seed_8)] <- 1:nrow(fs_table_raw)
fs_table_raw$Rank_coef_seed_9[order(-fs_table_raw$Abs_val_coef_seed_9)] <- 1:nrow(fs_table_raw)
...
# WE CHOOSE TO NOT REORDER TO SHOW TEH ROWS THAT SHOULD NOT NOT BE KEPT HAVE WEAK VALUE ACROSS MULTIPLES COLUMNS OF THE SAME TYPE 
# (eg : all coefs cols at zero, all seeds ranks very large as the mean of the rank)
fs_table_raw_full_table = fs_table_raw
...
filename1 = paste(start_of_filename_to_use,"_MeanRank10SeedsFullTableVranksasentallftsselectedornot.csv", sep='')
filename2 = paste(start_of_filename_to_use,"_MeanRank10SeedsFtsAndRanksOnlyVranksasentallftsselectedornot.csv", sep='')
# lets get to saving
