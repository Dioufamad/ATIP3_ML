...
# Version 4 (after corrections of Chloé). This version is what is obtained after deciding to focus on the top 4 FSs
...
write.xlsx(list_of_tables_for_previous_SIGs_not_passed, file = "Previous_41_SIGs_situation_after_SIGs_comp_with_Top4FSs.xlsx")
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ééé
...
final_table_V4_top4FSs_non_null_perc <-final_table_V4[which(final_table_V4$Percentage_selection_among_top4FSs > 0),]
# save it 
table_to_save_final_table_V4_top4FSs_non_null_perc <- final_table_V4_top4FSs_non_null_perc[order( -final_table_V4_top4FSs_non_null_perc$Percentage_selection_among_top4FSs ),]
filename_table_to_save_final_table_V4_top4FSs_non_null_perc = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/Multitask_learning/res_learning_tasks/outputs/runs/SIGs_perc_selection_among_top4_FSs_V4.xlsx"
# library(openxlsx)
# write.xlsx(table_to_save_final_table_V4_top4FSs_non_null_perc, filename_table_to_save_final_table_V4_top4FSs_non_null_perc)
...
#~~~~~~~~~~~~~~~~~~~lets start adding to the present SIGs all the coefs and values we wanted
#>>>>>>>>>>>>>>>>>>>>> lets  make the coefs and their means and rank
final_table_V5 = table_to_save_final_table_V4_top4FSs_non_null_perc
# We make 4 cols as "Mean Coef FS1-4"
final_table_V5$Mean_Coef_FS1_R04 <- NA
final_table_V5$Mean_Coef_FS2_MDA <- NA
final_table_V5$Mean_Coef_FS3_BMS <- NA
final_table_V5$Mean_Coef_FS4_MT <- NA
# + we supply the cols
list_fts_final_table_V5 = unique(as.character(final_table_V5$Features))
table_nnc_FS1 = list_fs_tables_retrieved_NNC[[1]] # lets get the table of coef per gene in each fs
table_nnc_FS2 = list_fs_tables_retrieved_NNC[[2]]
table_nnc_FS3 = list_fs_tables_retrieved_NNC[[3]]
table_nnc_FS4 = list_fs_tables_retrieved_NNC[[4]]
list_fts_table_nnc_FS1 = unique(as.character(table_nnc_FS1$Features)) # lets get the list of fts in each FS (to test later if the value is NA or not)
list_fts_table_nnc_FS2 = unique(as.character(table_nnc_FS2$Features))
list_fts_table_nnc_FS3 = unique(as.character(table_nnc_FS3$Features))
list_fts_table_nnc_FS4 = unique(as.character(table_nnc_FS4$Features))
for (a_ft_in_final_table_V5 in list_fts_final_table_V5){ # for test a_ft_in_final_table_V5 = list_fts_final_table_V5[1] ; a_ft_in_final_table_V5 =  "AANAT" ; a_ft_in_final_table_V5 =  "POF1B"
  if(a_ft_in_final_table_V5 %in% list_fts_table_nnc_FS1){
    mean_coef_ft_FS1 = table_nnc_FS1[which(table_nnc_FS1$Features %in% a_ft_in_final_table_V5),"Mean.Coefficient.Estimate.10.Seeds"] # lets get the val of the coef for the present gene
    final_table_V5[which(final_table_V5$Features %in% a_ft_in_final_table_V5),"Mean_Coef_FS1_R04"] = mean_coef_ft_FS1 # lets put the coef in place for the present gene
  }
  if(a_ft_in_final_table_V5 %in% list_fts_table_nnc_FS2){
    mean_coef_ft_FS2 = table_nnc_FS2[which(table_nnc_FS2$Features %in% a_ft_in_final_table_V5),"Mean.Coefficient.Estimate.10.Seeds"]
    final_table_V5[which(final_table_V5$Features %in% a_ft_in_final_table_V5),"Mean_Coef_FS2_MDA"] = mean_coef_ft_FS2 
  }
  if(a_ft_in_final_table_V5 %in% list_fts_table_nnc_FS3){
    mean_coef_ft_FS3 = table_nnc_FS3[which(table_nnc_FS3$Features %in% a_ft_in_final_table_V5),"Mean.Coefficient.Estimate.10.Seeds"]
    final_table_V5[which(final_table_V5$Features %in% a_ft_in_final_table_V5),"Mean_Coef_FS3_BMS"] = mean_coef_ft_FS3 
  }
  # case of the MT names
  MT_name_start = paste(a_ft_in_final_table_V5,"_in_",sep='')
  list_MT_names_corresponding = c()
  for (a_MT_name_known in list_fts_table_nnc_FS4){
    if(startsWith(a_MT_name_known, MT_name_start)){
      list_MT_names_corresponding = c(list_MT_names_corresponding,a_MT_name_known)
    }
  }
  table_nnc_FS4_rows_w_MT_name_start = table_nnc_FS4[which(table_nnc_FS4$Features %in% list_MT_names_corresponding),]
  numrows_table_nnc_FS4_rows_w_MT_name_start = nrow(table_nnc_FS4_rows_w_MT_name_start)
  if(numrows_table_nnc_FS4_rows_w_MT_name_start > 0){
    mean_coef_ft_FS4=mean(table_nnc_FS4_rows_w_MT_name_start$Mean.Coefficient.Estimate.10.Seeds, na.rm = TRUE)
    final_table_V5[which(final_table_V5$Features %in% a_ft_in_final_table_V5),"Mean_Coef_FS4_MT"] = mean_coef_ft_FS4
  }
# all fts in V5 have there mean coefs  
}
# We make a col as mean of the 4 Mean Coefs 1-4
final_table_V5$Mean_Coefs_FS1toFS4 <- NA # no need because the gene has been selected at least one so it has a value
final_table_V5$Mean_Coefs_FS1toFS4 <- rowMeans(subset(final_table_V5, select = c(Mean_Coef_FS1_R04, Mean_Coef_FS2_MDA,Mean_Coef_FS3_BMS,Mean_Coef_FS4_MT)), na.rm = TRUE)
# we make a col as rank mean coefs1-4
final_table_V5$Rank_Mean_Coefs_FS1toFS4 <- NA
final_table_V5$Rank_Mean_Coefs_FS1toFS4[order(-final_table_V5$Mean_Coefs_FS1toFS4)] <- 1:nrow(final_table_V5)
# save it 
table_to_save_final_table_V5 <- final_table_V5[order( -final_table_V5$Mean_Coefs_FS1toFS4 ),]
filename_table_to_save_final_table_V5 = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/Multitask_learning/res_learning_tasks/outputs/runs/Mean_on_coefs_SIGs_among_top4_FSs_V4.xlsx"
library(openxlsx)
write.xlsx(table_to_save_final_table_V5, filename_table_to_save_final_table_V5)
...
# We make 4 cols as % coef in sum mean cofs
final_table_V6$Share_Coef_FS1_R04 <- NA
final_table_V6$Share_Coef_FS2_MDA <- NA
final_table_V6$Share_Coef_FS3_BMS <- NA
final_table_V6$Share_Coef_FS4_MT <- NA
# get total means coefs in each FS
sum_all_coefs_FS1 = sum(table_nnc_FS1[, 'Mean.Coefficient.Estimate.10.Seeds'], na.rm = TRUE)
sum_all_coefs_FS2 = sum(table_nnc_FS2[, 'Mean.Coefficient.Estimate.10.Seeds'], na.rm = TRUE)
sum_all_coefs_FS3 = sum(table_nnc_FS3[, 'Mean.Coefficient.Estimate.10.Seeds'], na.rm = TRUE)
sum_all_coefs_FS4 = sum(table_nnc_FS4[, 'Mean.Coefficient.Estimate.10.Seeds'], na.rm = TRUE)
# lets supply the cols
list_fts_final_table_V6 = unique(as.character(final_table_V6$Features))
for (a_ft_in_final_table_V6 in list_fts_final_table_V6){ # test a_ft_in_final_table_V6 = list_fts_final_table_V6[1]
  share_FS1 = ((final_table_V6[which(final_table_V6$Features %in% a_ft_in_final_table_V6),"Mean_Coef_FS1_R04"])*100/sum_all_coefs_FS1) # get the values
  share_FS2 = ((final_table_V6[which(final_table_V6$Features %in% a_ft_in_final_table_V6),"Mean_Coef_FS2_MDA"])*100/sum_all_coefs_FS2)
  share_FS3 = ((final_table_V6[which(final_table_V6$Features %in% a_ft_in_final_table_V6),"Mean_Coef_FS3_BMS"])*100/sum_all_coefs_FS3)
  share_FS4 = ((final_table_V6[which(final_table_V6$Features %in% a_ft_in_final_table_V6),"Mean_Coef_FS4_MT"])*100/sum_all_coefs_FS4)
  
  final_table_V6[which(final_table_V6$Features %in% a_ft_in_final_table_V6),"Share_Coef_FS1_R04"] = share_FS1 # supply the cols
  final_table_V6[which(final_table_V6$Features %in% a_ft_in_final_table_V6),"Share_Coef_FS2_MDA"] = share_FS2 
  final_table_V6[which(final_table_V6$Features %in% a_ft_in_final_table_V6),"Share_Coef_FS3_BMS"] = share_FS3 
  final_table_V6[which(final_table_V6$Features %in% a_ft_in_final_table_V6),"Share_Coef_FS4_MT"] = share_FS4 
  
}
# We make a col as mean of the 4 Mean Coefs 1-4
final_table_V6$Mean_Shares_FS1toFS4 <- NA # no need because the gene has been selected at least one so it has a value
final_table_V6$Mean_Shares_FS1toFS4 <- rowMeans(subset(final_table_V6, select = c(Share_Coef_FS1_R04, Share_Coef_FS2_MDA,Share_Coef_FS3_BMS,Share_Coef_FS4_MT)), na.rm = TRUE)
# we make a col as rank mean coefs1-4
final_table_V6$Rank_Mean_Shares_FS1toFS4 <- NA
final_table_V6$Rank_Mean_Shares_FS1toFS4[order(-final_table_V6$Mean_Shares_FS1toFS4)] <- 1:nrow(final_table_V6)
#~~~~~~~~~~~~~~~~~~~
# save it 
table_to_save_final_table_V6 <- final_table_V6[order( -final_table_V6$Mean_Shares_FS1toFS4 ),]
filename_table_to_save_final_table_V6 = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/Multitask_learning/res_learning_tasks/outputs/runs/Share_on_coefs_SIGs_among_top4_FSs_V4.xlsx"
library(openxlsx)
write.xlsx(table_to_save_final_table_V6, filename_table_to_save_final_table_V6)

# save a version sorted on the perc
final_table_V7 = final_table_V6



final_table_V7 <- final_table_V6[order( -final_table_V6$Percentage_selection_among_top4FSs ),]
filename_7 = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/Multitask_learning/res_learning_tasks/outputs/runs/All_sorted_on_perc_of_representation_among_top4_FSs_V4.xlsx"
library(openxlsx)
write.xlsx(final_table_V7, filename_7)


library("readxl")
tab1 <- read_excel("/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/Multitask_learning/res_learning_tasks/outputs/runs/All_sorted_on_perc_of_representation_among_top4_FSs_V4.xlsx")


# We make a col as mean of the 4 Mean Coefs 1-4
tab1$Rank_R04 <- NA # no need because the gene has been selected at least one so it has a value
tab1$Rank_R04 <- rowMeans(subset(final_table_V6, select = c(Share_Coef_FS1_R04, Share_Coef_FS2_MDA,Share_Coef_FS3_BMS,Share_Coef_FS4_MT)), na.rm = TRUE)
# we make a col as rank mean coefs1-4
final_table_V6$Rank_Mean_Shares_FS1toFS4 <- NA
final_table_V6$Rank_Mean_Shares_FS1toFS4[order(-final_table_V6$Mean_Shares_FS1toFS4)] <- 1:nrow(final_table_V6)
#~~~~~~~~~~~~~~~~~~~
...
tab1$Rank_R04[order(-tab1$Mean_Coef_FS1_R04)] <- 1:nrow(tab1)
filename_of_table_to_save1 = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/Multitask_learning/res_learning_tasks/outputs/runs/tab1.xlsx"
library(openxlsx)
write.xlsx(tab1, filename_of_table_to_save1)
...
final_table_V8_top4FSs = final_table_V7_top4FSs
tab1 <-tab1[which(tab1$Mean_Coef_FS1_R04 > 0),]
# save it 
tab1 <- tab1[order( -tab1$Mean_Coef_FS1_R04 ),]
filename_of_table_to_save1 = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/Multitask_learning/res_learning_tasks/outputs/runs/SIGs_perc_selection_among_top4_FSs.xlsx"
library(openxlsx)
write.xlsx(table_to_save1, filename_of_table_to_save1)
