...
# Version 2 (after corrections of Chloé). This version gives you a table with 4 cols : strong_stringency_status, lenient_stringency_status, s_all, s_all_abs.
...
for (a_ft_in_table_V1 in list_fts_in_table_V1){ # for test use a_ft_in_table_V1 = list_fts_in_table_V1[3]
  ...
  # + lets compute s_all ( ##! it will be added to the column for it at the end of the loop...)
  ...
  # + lets inform the cols (high_string_status,lenient_string_status,s_all in that order)
  final_table_V1[which(final_table_V1$Features %in% a_ft_in_table_V1),"high_string_status"] = high_string_status
  final_table_V1[which(final_table_V1$Features %in% a_ft_in_table_V1),"lenient_string_status"] = lenient_string_status
...
final_table_V1_restrict_last3cols_stash = final_table_V1[c("Features","high_string_status","lenient_string_status","s_all","s_all_abs")]

# - lets add columns to identify the FS were each ft has existed
# final_table_V2 = final_table_V1
final_table_V2 = final_table_V1_restrict_last3cols_stash
# + lets load the table of presence/absence cols of fts across 7 FS by L1
path2file_pres_abs_fts_7_FSs2_L1Regr = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/Multitask_learning/res_learning_tasks/outputs/runs/presence_absence_table_for_FS_ST_L1_and_FS_MT_SGL1_7FSs_19535rows_8cols_GEX.csv"
table_pres_abs <- read.csv(file = path2file_pres_abs_fts_7_FSs2_L1Regr, header = TRUE)
# + restrict the table to only needed cols and sort it by perf of FS at the same time
names(table_pres_abs) # to see the cols to keep
table_pres_abs = table_pres_abs[c("Unique.features.across.all.initial.sets","FS_ST_L1_On_Remagus04","FS_ST_L1_On_MDAnderson_part1of2_310","FS_MT_SGL1_On_6_joined_dts",
                                           "FS_ST_L1_On_BMS_Horak_2013","FS_ST_L1_On_Osaka_Miyake_2012","FS_ST_L1_On_Remagus02","FS_ST_L1_On_Fudan_MAQC2BR")]
# rename columns for easier names
names(table_pres_abs) <- c("Features_bis","FS_Remagus04","FS_MDAnderson_part1of2_310","FS_MT_6_dts",
                                     "FS_BMS_Horak_2013","FS_Osaka_Miyake_2012","FS_Remagus02","FS_Fudan_MAQC2BR")
names(table_pres_abs)
# + Optionnaly, we can restrict to only the FS needed
...
#                                   "FS_BMS_Horak_2013","FS_Osaka_Miyake_2012","FS_Remagus02","FS_Fudan_MAQC2BR")]
# #<<<<<<<<<<<<
# #>>>>>>>>>>>for another FS, uncomment and modify
# table_pres_abs = table_pres_abs[c("Features_bis","FS_Remagus04","FS_MDAnderson_part1of2_310","FS_MT_6_dts",
#                                   "FS_BMS_Horak_2013","FS_Osaka_Miyake_2012","FS_Remagus02","FS_Fudan_MAQC2BR")]
# #<<<<<<<<<<<<<<<<<
