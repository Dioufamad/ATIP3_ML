...
# Objective : same as "UnifiedMatrixDesign2_CommonFts_6cohorts_maker.py" (each standalone dataset has to be fit+transform before joined)
# but with all fts
...
df_file1_b4_std = df_file1.copy()
df_file2_b4_std = df_file2.copy()
df_file3_b4_std = df_file3.copy()
df_file4_b4_std = df_file4.copy()
df_file5_b4_std = df_file5.copy()
# df_file6_b4_std = df_file6.copy()
df_file7_b4_std = df_file7.copy()
list_of_dfs_b4_std = [df_file1_b4_std,df_file2_b4_std,df_file3_b4_std,df_file4_b4_std,df_file5_b4_std,df_file7_b4_std]
# >>>> standardize the values across samples for each feature


# transform for each of the dataset its fts frame
list_of_dfs_2_transform = [df_file1,df_file2,df_file3,df_file4,df_file5,df_file7]
...
for a_full_df in list_of_dfs_2_transform: # for test : a_full_df = list_of_dfs_2_transform[0]
    ...
    # get the list of fts col to change
    list_all_cols_a_full_df = list(a_full_df.columns)
    list_all_cols_a_full_df.remove("Y_pCR")
    list_all_cols_a_full_df.remove("cohort")
    # get the frame of fts to change
    fts_frame_2_change = a_full_df.loc[:, list_all_cols_a_full_df] # or model_fts_frame_4_scaler = df_file1[list_all_cols_a_full_df].copy()
    # fit the scaler to one dataset fts frame
    scaler.fit(fts_frame_2_change)
    fts_frame_scaled = scaler.transform(fts_frame_2_change)
    a_full_df.loc[:, list_all_cols_a_full_df] = fts_frame_scaled
...
# Columns: 87442 entries, DDR1 /// MIR4640_in_GSE41998 to cohort
# dtypes: float64(87440), int64(1), object(1)
# memory usage: 152.8+ MB


#>>>> saving the df
fullname_file_of_df_joined_new_design2 = "/data_acquisition/1_pipelines_or_tasks/JoinNewDesign2AllFtsof6_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_6cohortname.csv"
