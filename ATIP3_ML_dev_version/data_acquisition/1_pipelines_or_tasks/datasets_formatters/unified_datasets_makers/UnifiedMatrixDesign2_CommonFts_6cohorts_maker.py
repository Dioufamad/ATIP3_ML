...
# Objective : same as "UnifiedMatrixDesign1_CommonFts_6cohorts_maker.py" but with each standalone dataset has to be fit+transform before joined
# A script to create a unified matrix design for analysis of multiples populations that share the same response (design 2)
# version for unifying 6 cohorts
...
# NB2 ; previously all these datasets had cols that helped to restrict to taxanes treated only samples
# NB3 : each standalone dataset has to be fit+transform before joined
...
# NB : the df_file6 ie GSE23988 is not included in this reunion due to having only 4 samples and then it disturbed any CV done with more than 4 folds and we at least need to do 5XCV best is 10XCV
# hence 6 cohorts (out of 9)

#>>>>>>>>>>>>>> Loading type 1 datsets (6)
#---set the name of the sheet where the data of the cohort is
filepath_of_ml_dataset1= "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_warehouse/outputs/atip3_ml_dataset_type1/GSE41998_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_cohortname.csv"
filepath_of_ml_dataset2= "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_warehouse/outputs/atip3_ml_dataset_type1/GSE26639_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_cohortname.csv"
filepath_of_ml_dataset3= "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_warehouse/outputs/atip3_ml_dataset_type1/GSE32646_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_cohortname.csv"
filepath_of_ml_dataset4= "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_warehouse/outputs/atip3_ml_dataset_type1/GSE25055_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_cohortname.csv"
filepath_of_ml_dataset5= "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_warehouse/outputs/atip3_ml_dataset_type1/GSE20194_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_cohortname.csv"
# filepath_of_ml_dataset6= "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_warehouse/outputs/atip3_ml_dataset_type1/GSE23988_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_cohortname.csv"
filepath_of_ml_dataset7= "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_warehouse/outputs/atip3_ml_dataset_type1/GSE63471_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_cohortname.csv"
sep_in_file = ","
...
p = [list_all_cols_df_file1,list_all_cols_df_file2,list_all_cols_df_file3,list_all_cols_df_file4,list_all_cols_df_file5,list_all_cols_df_file7]
result = set(p[0])
for s in p[1:]:
    result.intersection_update(s)
print("Number of columns retained as common : ",len(result)) # 12036 ie 12304 fts
...
list_of_common_of_dfs_b4_std = [common_of_df_file1_b4_std,common_of_df_file2_b4_std,common_of_df_file3_b4_std,common_of_df_file4_b4_std,common_of_df_file5_b4_std,common_of_df_file7_b4_std]
# >>>> standardize the values across samples for each feature



# transform for each of the dataset its fts frame
list_of_dfs_2_transform = [common_of_df_file1,common_of_df_file2,common_of_df_file3,common_of_df_file4,common_of_df_file5,common_of_df_file7]
num_fts_frame_changed = 0
# - way 1 : replacing all fts cols at once
for a_full_df in list_of_dfs_2_transform: # for test : a_full_df = list_of_dfs_2_transform[0]
    # About the model of common fts frame to use to fit the scaler : here, each dataset is fitted on itself and transform itself (a classic fit+transform)
    # (This is the heart of the 'UnifiedMatrixDesign2 : fit+transforming each dataset before joining them)
    # introduce the naive scaler
    scaler = StandardScaler()
    fts_frame_2_change = a_full_df.loc[:, list_of_common_fts] # or model_fts_frame_4_scaler = common_of_df_file1[list_of_common_fts].copy()
    # fit the scaler to one dataset fts frame
    scaler.fit(fts_frame_2_change)
...
df_joined_new_design2 = pd.merge(df_joined_fts_only_zeroed, df_joined_resp_only, left_index=True, right_index=True)
...
list_all_cols_df_joined_new_design2 = list(df_joined_new_design2.columns)
list_all_cols_df_joined_new_design2.remove("Y_pCR")
list_all_cols_df_joined_new_design2.remove("cohort")
df_joined_new_design2_4_image = df_joined_new_design2.copy()
df_joined_new_design2_4_image = df_joined_new_design2_4_image[list_all_cols_df_joined_new_design2]
df_joined_new_design2_4_image[df_joined_new_design2_4_image != 0.0] = 999
...
plt.imshow(df_joined_new_design2_4_image, cmap='viridis', aspect='auto')
plt.colorbar()
# succession of cohorts on the Y axis is needed
list_of_cohorts_in_Yaxis_of_data_image = df_joined_new_design2["cohort"].unique()
...
# list_all_cols_df_joined_new_design2_sorted = sorted(list_all_cols_df_joined_new_design2)
# list_all_cols_df_joined_new_design2_sorted_plus2 = list_all_cols_df_joined_new_design2_sorted + ["Y_pCR","cohort"]
# df_joined_new_design2_ftssorted = df_joined_new_design2[list_all_cols_df_joined_new_design2_sorted_plus2]

# >>>>>> check the datatypes
df_joined_new_design2.info()
# Index: 229 entries, GSM1030229 to GSM1550523
# Columns: 72206 entries, A1CF_in_GSE41998 to cohort
# dtypes: float64(72204), int64(1), object(1)
# memory usage: 126.2+ MB


#>>>> saving the df
fullname_file_of_df_joined_new_design2 = "/data_acquisition/1_pipelines_or_tasks/JoinNewDesign2CommonFtsof6_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_6cohortname.csv"
df_joined_new_design2.to_csv(fullname_file_of_df_joined_new_design2, header=True)
print("File saved !")
#>>>>>>>>>>>>>>
