...
# - the dimensions of the data struct to save to fit in the file
num_rows<-nrow(data_struct2save)
num_cols<-ncol(data_struct2save)
# - the file name definition
outfilename = paste(cohort,"_",myGSE,"_",part_of_probesets_kept,"_",version_to_save,"_",num_rows,"x",num_cols,"_GEX.csv", sep='')
# before saving print the num of cols or rows not defined
num_not_defined_rownames<-sum(is.nan(rownames(ex)))
num_not_defined_colnames<-sum(is.nan(colnames(ex)))
paste("# rows not defined : ",num_not_defined_rownames,". # cols not defined :",num_not_defined_colnames,".", sep='')
