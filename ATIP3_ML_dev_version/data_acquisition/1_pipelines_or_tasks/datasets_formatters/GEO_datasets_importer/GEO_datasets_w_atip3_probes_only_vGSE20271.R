...
# write.csv(peak_at_names_in_phdata, "GSE20271_phdata_entries_repaired.csv", row.names=T) # ... save the file) # uncomment this to save the file 
# do the repairs by hand
# reload the file after the repairs
path2filerepaired1 = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/GSE20271_phdata_entries_repaired.csv"
...
myphcolsDesired <- c("geo_accession","characteristics_ch1.19","characteristics_ch1.12","characteristics_ch1.14","characteristics_ch1.15","characteristics_ch1")
peak_at_names_in_phdata_restricted <- peak_at_names_in_phdata[, myphcolsDesired]
# rename the cols for more controlable colnames
names(peak_at_names_in_phdata_restricted) <- c("geo_accession","treatment","ER_status","PR_status","HER2_status","pCR_status") # these two lines will rename the columns
...
peak_at_names_in_phdata_restricted$treatment <- as.character(peak_at_names_in_phdata_restricted$treatment) 
...
unique(peak_at_names_in_phdata_restricted$treatment) # line of the treatments info to make two lists contains the unwanted info
...
peak_at_names_in_phdata_restricted$ER_status[which((peak_at_names_in_phdata_restricted$ER_status %in% c("er status: P")))]<-"positive"
peak_at_names_in_phdata_restricted$ER_status[which((peak_at_names_in_phdata_restricted$ER_status %in% c("er status: N")))]<-"negative"
peak_at_names_in_phdata_restricted$PR_status[which((peak_at_names_in_phdata_restricted$PR_status %in% c("pr status: P")))]<-"positive"
peak_at_names_in_phdata_restricted$PR_status[which((peak_at_names_in_phdata_restricted$PR_status %in% c("pr status: N")))]<-"negative"
peak_at_names_in_phdata_restricted$HER2_status[which((peak_at_names_in_phdata_restricted$HER2_status %in% c("her 2 status: P")))]<-"positive"
peak_at_names_in_phdata_restricted$HER2_status[which((peak_at_names_in_phdata_restricted$HER2_status %in% c("her 2 status: N")))]<-"negative"
peak_at_names_in_phdata_restricted$pCR_status[which((peak_at_names_in_phdata_restricted$pCR_status %in% c("pcr or rd: pCR")))]<-"Yes"
peak_at_names_in_phdata_restricted$pCR_status[which((peak_at_names_in_phdata_restricted$pCR_status %in% c("pcr or rd: RD")))]<-"No"
...
# peak_at_names_in_phdata_restricted before restriction on patients treated with taxanes (TP) has  178 samples
# restrict to taxanes treated patients 
peak_at_names_in_phdata_restricted_TP <- peak_at_names_in_phdata_restricted[which((peak_at_names_in_phdata_restricted$treatment %in% c("treatment received (1=fac, 2=t/fac): 2"))),] # 91 samples
# restrict to TP that are TNBC
peak_at_names_in_phdata_restricted2TNBC <- peak_at_names_in_phdata_restricted_TP[which(peak_at_names_in_phdata_restricted_TP$TNBC %in% c("yes")),] ##! to save  # 31 samples
