...
myGSE = "GSE23988" # used also to set the dataset's name
myGPL = "GPL96"
...
# write.csv(peak_at_names_in_phdata, "GSE20271_phdata_entries_repaired.csv", row.names=T) # ... save the file)
...
myphcolsDesired <- c("geo_accession","characteristics_ch1.2","characteristics_ch1")
peak_at_names_in_phdata_restricted <- peak_at_names_in_phdata[, myphcolsDesired]
# rename the cols for more controlable colnames
names(peak_at_names_in_phdata_restricted) <- c("geo_accession","ER_status","pCR_status") # these two lines will rename the columns
...
peak_at_names_in_phdata_restricted$ER_status[which((peak_at_names_in_phdata_restricted$ER_status %in% c("er positive vs negative: ERpos")))]<-"positive"
peak_at_names_in_phdata_restricted$ER_status[which((peak_at_names_in_phdata_restricted$ER_status %in% c("er positive vs negative: ERneg")))]<-"negative"
peak_at_names_in_phdata_restricted$pCR_status[which((peak_at_names_in_phdata_restricted$pCR_status %in% c("pcr.v.rd: pCR")))]<-"Yes"
peak_at_names_in_phdata_restricted$pCR_status[which((peak_at_names_in_phdata_restricted$pCR_status %in% c("pcr.v.rd: RD")))]<-"No"
# restrict to taxanes treated patients (not needed all are treated with taxanes) # 61 samples
# peak_at_names_in_phdata_restricted_b4_tax_restriction <- peak_at_names_in_phdata_restricted  # 178 samples
# peak_at_names_in_phdata_restricted <- peak_at_names_in_phdata_restricted[which((peak_at_names_in_phdata_restricted$treatment %in% c("treatment received (1=fac, 2=t/fac): 2"))),] # 91 samples
...
# make the TNBC column (not possible because PR_status is not given here)
...
# peak_at_names_in_phdata_restricted2TNBC <- peak_at_names_in_phdata_restricted[which(peak_at_names_in_phdata_restricted$TNBC %in% c("yes")),] ##! to save  # 31 samples
