...
myGSE = "GSE20194" # used also to set the dataset's name
myGPL = "GPL96"
...
#-------------------some columns we need have their cells moved to left or right (we decide to take a knee and repair the ph data table then reload it
# saving a file to repair it (done for the ph data entries)
# write.csv(peak_at_names_in_phdata, "GSE20194_phdata_entries_repaired.csv", row.names=T) # ... save the file) #uncomment to rewrite file
# do the repairs by hand
# reload the file after the repairs
path2filerepaired1 = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/GSE20194_phdata_entries_repaired.csv"
peak_at_names_in_phdata <- read.csv(file = path2filerepaired1, header = TRUE)
rownames(peak_at_names_in_phdata) = peak_at_names_in_phdata$X
#------------------
# - see the cols names to copy what to keep
colnames(peak_at_names_in_phdata)
# - make the restriction 
myphcolsDesired <- c("geo_accession","characteristics_ch1.14","characteristics_ch1.15","characteristics_ch1.3","characteristics_ch1.5","characteristics_ch1.10","characteristics_ch1.4")
peak_at_names_in_phdata_restricted <- peak_at_names_in_phdata[, myphcolsDesired]
# rename the cols for more controlable colnames
names(peak_at_names_in_phdata_restricted) <- c("geo_accession","treatment_code","treatment_comments","ER_status","PR_status","HER2_status","pCR_status") # these two lines will rename the columns
...
peak_at_names_in_phdata_restricted$treatment_code <- as.character(peak_at_names_in_phdata_restricted$treatment_code) 
peak_at_names_in_phdata_restricted$treatment_comments <- as.character(peak_at_names_in_phdata_restricted$treatment_comments) 
...
unique(peak_at_names_in_phdata_restricted$treatment_code) # these two lines of the treatments info to make two lists contains the unwanted info
unique(peak_at_names_in_phdata_restricted$treatment_comments) 
...
peak_at_names_in_phdata_restricted$ER_status[which((peak_at_names_in_phdata_restricted$ER_status %in% c("er_status: P")))]<-"positive"
peak_at_names_in_phdata_restricted$ER_status[which((peak_at_names_in_phdata_restricted$ER_status %in% c("er_status: N")))]<-"negative"
peak_at_names_in_phdata_restricted$PR_status[which((peak_at_names_in_phdata_restricted$PR_status %in% c("pr_status: P")))]<-"positive"
peak_at_names_in_phdata_restricted$PR_status[which((peak_at_names_in_phdata_restricted$PR_status %in% c("pr_status: N")))]<-"negative"
peak_at_names_in_phdata_restricted$HER2_status[which((peak_at_names_in_phdata_restricted$HER2_status %in% c("her2 status: P")))]<-"positive"
peak_at_names_in_phdata_restricted$HER2_status[which((peak_at_names_in_phdata_restricted$HER2_status %in% c("her2 status: N")))]<-"negative"
peak_at_names_in_phdata_restricted$pCR_status[which((peak_at_names_in_phdata_restricted$pCR_status %in% c("pcr_vs_rd: pCR")))]<-"Yes"
peak_at_names_in_phdata_restricted$pCR_status[which((peak_at_names_in_phdata_restricted$pCR_status %in% c("pcr_vs_rd: RD")))]<-"No"
# restrict to taxanes treated patients 
list_non_taxol_treatment_codes <- c("treatment code: FAC","treatment code: FEC","treatment code: NA",NA)
list_non_taxol_treatment_comments <- c("treatments comments: FAC x 6","treatments comments: FEC x 4",NA)
peak_at_names_in_phdata_restricted$treatment <- "NOT_CHANGED_YET"
`%notin%` <- Negate(`%in%`)
peak_at_names_in_phdata_restricted$treatment[which((peak_at_names_in_phdata_restricted$treatment_code %notin% list_non_taxol_treatment_codes)
                                              &(peak_at_names_in_phdata_restricted$treatment_comments %notin% list_non_taxol_treatment_comments))]<-"treatment arm: Paclitaxel"

# get rid of the columns not needed
colnames(peak_at_names_in_phdata_restricted)
myphcolsDesired1 <- c("geo_accession","treatment","ER_status","PR_status","HER2_status","pCR_status")
peak_at_names_in_phdata_restricted <- peak_at_names_in_phdata_restricted[, myphcolsDesired1]
...
# peak_at_names_in_phdata_restricted before restriction on patients treated with taxanes (TP) has  278 samples
# restrict to taxanes treated patients 
peak_at_names_in_phdata_restricted_TP <- peak_at_names_in_phdata_restricted[which((peak_at_names_in_phdata_restricted$treatment %in% c("treatment arm: Paclitaxel"))),] # 257 samples
# restrict to TP that are TNBC
peak_at_names_in_phdata_restricted2TNBC <- peak_at_names_in_phdata_restricted_TP[which(peak_at_names_in_phdata_restricted_TP$TNBC %in% c("yes")),] ##! to save  # 62 samples
...
# 4) make a restricted version (to all samples (taxanes and non taxanes treated both) of the expression set
...
num_final_of_taxanes_samples_in_cohort <-nrow(peak_at_names_in_phdata_restricted_TP) # or nrow(peak_at_names_in_phdata_restricted)
