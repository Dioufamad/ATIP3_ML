...
myGSE = "GSE63471" # used also to set the dataset's name
myGPL = "GPL571" 
# - uncomment one of these 2 following lines to choose a version
...
##### Optional : Make a feature data restricted to info needed later in feature selections analysis 
# (classes of each gene with all necessary info about the gene)

# - a good table to start with is peak_at_names_in_ftdata
# we restrict it to only the cols we would like to keep
base_ftdata_table = peak_at_names_in_ftdata
colnames(base_ftdata_table) # to display the colnames and copy paste the needed ones, in the order we would like to have them (from most needed info to least needed info)
list_cols_2keep_from_base_table <- c("Gene Symbol","Gene Title",
                                     "Gene Ontology Biological Process","Gene Ontology Cellular Component","Gene Ontology Molecular Function",
                                     "ENTREZ_GENE_ID","GB_ACC","ID",
                                     "Annotation Date","Sequence Source")
peak_at_names_in_ftdata_v1 = base_ftdata_table[, list_cols_2keep_from_base_table]
# we restrict it to only the rows where there is a best probeset
# a good table to use for the selection of the rows to keep is best_probes_repo_w_rows_order_of_ex1 (best probeset and GS in the same order as the rows was in the values table)
colnames(best_probes_repo_w_rows_order_of_ex1) # to display the colnames
list_of_GSs_2keep <- unique(as.character(best_probes_repo_w_rows_order_of_ex1$"Gene Symbol"))
list_of_best_probesets_2keep <- unique(as.character(best_probes_repo_w_rows_order_of_ex1$"ID"))
peak_at_names_in_ftdata_v2 <- peak_at_names_in_ftdata_v1[which((peak_at_names_in_ftdata_v1$`Gene Symbol` %in% list_of_GSs_2keep)
                                                               &(peak_at_names_in_ftdata_v1$ID %in% list_of_best_probesets_2keep)),]

names(peak_at_names_in_ftdata_v2) <- c("Gene_Symbol","Gene_fullname_aka_Title",
                                       "GO_Biological_Process","GO_Cellular_Component","GO_Molecular_Function",
                                       "ENTREZ_GENE_ID","GB_ACC","Best_probeset_ID",
                                       "Annotation_Date","Sequence_Source") # rename the columns (to change ID into best_probeset and put colnames without spaces for future easier scripting)
# we use now a regex system to simplify the info in the GO columns
# (source : https://stackoverflow.com/questions/39086400/extracting-a-string-between-other-two-strings-in-r)
