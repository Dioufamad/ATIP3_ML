...
#------> decision of a merge of clusters
merge_clusters_low_and_med_high = "yes"
# merge_clusters_low_and_med_high = "no"

# ### Loading data sent by Sylvie
#---set the name of the sheet where the data of the cohort is
filepath_1cohort_gex_data_ATIP3probesonly= "/data_warehouse/outputs/atip3_ml_dataset_type1/GSE20271_ATIP3probesetsonly_probesnames_3x178_GEX.csv"
sep_in_file = ","
...
seaborn_cluster = sns.clustermap(df_X_scaled.drop(columns=list_of_cols_to_drop_from_df_X_scaled),
                                 ...
                                 figsize=(5, 8))
# merge the cluster low and the med-high or not:
if merge_clusters_low_and_med_high == "no":
    ...
    # clusters_placement_dictionary ={3 : "low", 0 : "low_med", 2 : "med_high", 1 : "high"} # use this for 4 clusters {0: 'green', 1: 'blue', 2: 'red', 3: 'khaki'}
    # NB : the low half of the cluster 2/4 can be added to the cluster 1 is we do 5 clusters
    clusters_placement_dictionary ={3 : "low", 2 : "low", 1 : "low_med", 4 : "med_high", 0 : "high"} # use this for 5 clusters {0: 'green', 1: 'blue', 2: 'red', 3: 'khaki', 4: 'orange'}
    df_X_scaled['ATIP3_class_hm'] = df_X_scaled['cluster'].map(clusters_placement_dictionary)
    # add the columns of the hormonal profiles
    filepath_1cohort_ph_data_ATIP3probesonly= "/data_warehouse/outputs/atip3_ml_dataset_type1/GSE20271_ATIP3probesetsonly_probesnames_178x7_ph.csv"
    sep_in_file = ","
    ...
    old_cols_list = ["treatment","HER2_status","ER_status","PR_status","TNBC","pCR_status"]
    new_cols_list = ["treatment","HER2_status","RO_status","PR_status","TN_status","RCH"]
    ...
    all_row_colors1 = pd.concat([TN_row_colors,pCR_row_colors,cluster_row_colors],axis=1) # ,RO_row_colors,PR_row_colors,HER2_row_colors removed to be added for more info on hormonal statuses
    # Plot the cluster heatmap + dendrogram again but this time with the additional hormonal row colors
    list_of_cols_to_drop_from_joined_table = ['cluster',"ATIP3_class_hm","treatment","HER2_status","RO_status","PR_status","TN_status","RCH"]
    ...
    # fullname_file_w_treatment_and_clusters = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/GSE20271_ATIP3probesetsonly_InfoTreatmentCluster.csv"
    ...
    df_X_scaled_joined = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2")]
    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

    # >>>>>>> For the cohort analysed, give a report
    print("For the full cohort analysed, we found :")
    for a_cluster_num in range(num_clusters_wanted):
        print("- a cluster of size :",len(df_X_scaled_joined_all[df_X_scaled_joined_all.cluster==a_cluster_num]),"colored in",my_list_of_clusters_colors[a_cluster_num])
    # >>>>>>> For the cohort analysed, give a report only on the samples from taxanes treated samples
    print("For the cohort TP part analysed, we found :")
    for a_cluster_num in range(num_clusters_wanted):
    ...
    # tab_pCR = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.RCH==1)] # the pCR status in the TP (2 groups)
    # tab_NpR = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.RCH==0)]
    ...
    # tab_TN_low = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low")] # the TNBC & low
    # tab_TN_low_med = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low_med")] # the TNBC & low_med
    # tab_TN_med_high = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="med_high")] # the TNBC & med_high
    # tab_TN_high = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="high")] # the TNBC & high
    # tab_TN_low_pCR = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.RCH==1) &(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low")] # the TNBC & low with pCR
    # tab_TN_low_med_pCR = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.RCH==1) &(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low_med")] # the TNBC & low_med with pCR
    # tab_TN_low_NpR = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.RCH==0) &(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low")] # the TNBC & low with NpR
    # tab_TN_low_med_NpR = df_X_scaled_joined[(df_X_scaled_joined.treatment=="treatment received (1=fac, 2=t/fac): 2") & (df_X_scaled_joined.RCH==0) &(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low_med")] # the TNBC & low_med with NpR
    # #>>>>>>>>>>>>>>>>>>>>>>>>>>
    print("For the cohort analysed, the population stratification is :")
    ...
    tab_NpR_med_high = df_X_scaled_joined[(df_X_scaled_joined.RCH==0) &(df_X_scaled_joined.ATIP3_class_hm=="med_high")]
    tab_NpR_high = df_X_scaled_joined[(df_X_scaled_joined.RCH==0) &(df_X_scaled_joined.ATIP3_class_hm=="high")]
    tab_TN_low = df_X_scaled_joined[(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low")] # the TNBC & low
    tab_TN_low_med = df_X_scaled_joined[(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low_med")] # the TNBC & low_med
    tab_TN_med_high = df_X_scaled_joined[(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="med_high")] # the TNBC & med_high
    tab_TN_high = df_X_scaled_joined[(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="high")] # the TNBC & high
    tab_TN_low_pCR = df_X_scaled_joined[(df_X_scaled_joined.RCH==1) &(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low")] # the TNBC & low with pCR
    tab_TN_low_med_pCR = df_X_scaled_joined[(df_X_scaled_joined.RCH==1) &(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low_med")] # the TNBC & low_med with pCR
    tab_TN_low_NpR = df_X_scaled_joined[(df_X_scaled_joined.RCH==0) &(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low")] # the TNBC & low with NpR
    tab_TN_low_med_NpR = df_X_scaled_joined[(df_X_scaled_joined.RCH==0) &(df_X_scaled_joined.TN_status=="yes") &(df_X_scaled_joined.ATIP3_class_hm=="low_med")] # the TNBC & low_med with NpR
...
elif merge_clusters_low_and_med_high == "yes":
    #>>>>>>>>>>>>>>>>>>>>>#>>>>>>>>>>>>>>>>>>>>>other option of the merge
    # add a column of the clusters names (low, med, high or low,low_med, med_high, high) # choosen clusters following what we see
    print("The attributed colors to clusters were : ",my_palette_for_row_colors) # shows {0: 'green', 1: 'blue', 2: 'red', 3: 'khaki'}
    # clusters_placement_dictionary ={0 : "med", 1 : "low", 2 : "high"} # use this for 3 clusters
    # clusters_placement_dictionary ={3 : "low", 0 : "low_med", 2 : "med_high", 1 : "high"} # use this for 4 clusters {0: 'green', 1: 'blue', 2: 'red', 3: 'khaki'}
    # NB : the low half of the cluster 2/4 can be added to the cluster 1 is we do 5 clusters
    clusters_placement_dictionary ={3 : "low", 2 : "low", 1 : "low_med", 4 : "low", 0 : "high"} # use this for 5 clusters {0: 'green', 1: 'blue', 2: 'red', 3: 'khaki', 4: 'orange'}
    df_X_scaled['ATIP3_class_hm'] = df_X_scaled['cluster'].map(clusters_placement_dictionary)
    # add the columns of the hormonal profiles
    filepath_1cohort_ph_data_ATIP3probesonly= "/data_warehouse/outputs/atip3_ml_dataset_type1/GSE20271_ATIP3probesetsonly_probesnames_178x7_ph.csv"
    sep_in_file = ","
    ...
    chosen_box_pairs=[
        (("212093_s_at", "low"), ("212093_s_at", "low_med")),
        (("212093_s_at", "low_med"), ("212093_s_at", "high")),
        (("212095_s_at", "low"), ("212095_s_at", "low_med")),
        (("212095_s_at", "low_med"), ("212095_s_at", "high")),
        (("212096_s_at_(best_probeset)", "low"), ("212096_s_at_(best_probeset)", "low_med")),
        (("212096_s_at_(best_probeset)", "low_med"), ("212096_s_at_(best_probeset)", "high")),
        ]
    ax = sns.boxplot(data=chosen_data, x=chosen_x, order=chosen_x_order, y=chosen_y, hue=chosen_hue, hue_order=chosen_hue_order)  # df_file_phRUN PLOT
    ax, test_results = add_stat_annotation(ax, data=chosen_data, x=chosen_x, y=chosen_y, order=chosen_x_order,hue=chosen_hue, hue_order=chosen_hue_order,
                                       box_pairs=chosen_box_pairs, test='Mann-Whitney', text_format='star', loc='inside', verbose=2)
    ...
    print("please setup the merge you want to realise on the clusters")
