# This is the V2.0 of the script "gridsearchcv_sklearn_MCC_L2.py"
# Objective : run sklearn L2 log reg on differents datasets (priority is the 1 cohort datasets)
# New possibilities :
...
for a_seed in list_seeds:  # test a_seed = 0, # a_seed = 1
    ...
    modelselector_by_GSCV = model_selection.GridSearchCV(linear_model.LogisticRegression(penalty='l2', solver='saga', random_state=a_seed, class_weight="balanced", fit_intercept=False,max_iter=200,tol=0.01), param_grid, scoring=grid_scorer, cv=10, n_jobs=-1) # same as before but without stating the refit
    # - fit lasso
