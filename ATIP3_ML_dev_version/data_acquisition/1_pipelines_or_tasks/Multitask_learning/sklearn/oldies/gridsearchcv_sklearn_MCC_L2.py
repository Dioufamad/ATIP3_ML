...
filepath_of_ml_dataset= "/data_acquisition/1_pipelines_or_tasks/GSE63471_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_cohortname.csv"
...
for a_seed in list_seeds:  # test a_seed = 0, # a_seed = 1
    ...
    lasso_gs = model_selection.GridSearchCV(linear_model.LogisticRegression(penalty='l2', solver='saga', random_state=a_seed, class_weight="balanced", fit_intercept=False,max_iter=200,tol=0.01), param_grid, scoring=grid_scorer, cv=10, n_jobs=-1)  # same as before but without stating the refit
    # - fit lasso
