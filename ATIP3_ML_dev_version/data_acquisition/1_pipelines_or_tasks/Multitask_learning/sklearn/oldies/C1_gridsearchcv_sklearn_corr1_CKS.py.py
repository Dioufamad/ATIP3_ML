...
# np.random.seed(42) #? to keep evrything stable outside of the seeds
...
#  update the list of lambda values to explore with the known lambdas value across some APIs.
# we cite  1.0 for sklearn, 0.1 for SPAMS,
...
for a_seed in list_seeds:  # test a_seed = 0
    ...
    lasso_gs = model_selection.GridSearchCV(linear_model.LogisticRegression(penalty='l1', solver='liblinear',random_state=a_seed), param_grid, scoring=cks_as_scorer, cv=10, n_jobs=-1) # same as before but without stating the refit (beta 2)
    # - fit lasso
