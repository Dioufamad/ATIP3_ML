...
filepath_of_ml_dataset= "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/JoinNewDesign1CommonFtsof6_FSidea1thinking1or3_ml_dataset_taxanesonly_ATIP3lowonly_GEX_pCR_6cohortname.csv"
...
length_list_fts_no_tag_cohort = len(list_all_fts_no_tag_cohort) # not needed
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>> all hyperparamerters
regularization_used = "SpamsSparseGroupL1LogReg"
...
# param_grid = {"lambda1": reg_strength_spaceplus1value_sorted}
# param_grid = {"lambda2": reg_strength_spaceplus1value_sorted}
# param_grid = {"lambda3": reg_strength_spaceplus1value_sorted}
# to test param grid use this
...
class CustomSPAMSestimator(base.BaseEstimator, base.ClassifierMixin): # rest of name LossIsSQUARERegulIsSPARSEGROUPLASSOL2V1
    """
    ...
    def fit(self, X, y):
        ...
        self.y_ = y
        # Fitting
        # X_sparse = sparse.csc_matrix(self.X_)  # np.asfortranarray(self.X_) # old version
        # y_asfa = np.asfortranarray(self.y_.reshape((self.y_.shape[0], 1))) # old version
        # m = X.shape(0)
        # n = X.shape(1)
        # t=m*n
        # X_sparse = np.asfortranarray(self.X_, dtype ='float') # present versions 2
        # y_asfa = np.asfortranarray(self.y_, dtype ='float') # present versions 2
        X_sparse = np.asfortranarray(self.X_.reshape(-1, 1),dtype ='float') # version 3
        y_asfa = np.asfortranarray(self.y_.reshape((self.y_.shape[0], 1)))  # version 3
        w_init = np.zeros((X_sparse.shape[1], 1), order="F")
        w = spams.fistaFlat(y_asfa, X_sparse, W0=w_init, loss='square', regul='l1', groups=self.groups, lambda1=self.lambda1, lambda2=self.lambda2)
...
# >>>>>>>>>>>> Let 's run a multitask regularized regressions on joined datasets with SPAMS on 10 seeds
...
for a_seed in list_seeds:  # test a_seed = 0, # a_seed = 1
    ...
    # - define the grid of params values to explore
    # param_grid = {'C': reg_strength_space1value_sorted_inverts}  # define the grid of params and params values to explore # (old version used for sklearn)
    # param_grid is placed before the definition of the custom estimator
    # - define the gridsearchcv
    lasso_gs = model_selection.GridSearchCV(CustomSPAMSestimator(groups=groups_in_data), param_grid, scoring=grid_scorer, cv=10, n_jobs=-1) # groups=groups_in_data # same as before but without stating the refit
    # - fit lasso
