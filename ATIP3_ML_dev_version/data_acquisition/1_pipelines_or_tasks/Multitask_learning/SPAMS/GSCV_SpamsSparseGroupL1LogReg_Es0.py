# making an estimator that will use a joined dataset (common fts)
# - used for a joined of 7 cohorts (tag_cohort = "JoinNewDesign2CommonFtsof6")
# - used for a joined of 3 cohorts (tag_cohort = "JoinNewDesign2CommonFtsof3oftop4FSsL1")
...
# scheme_used = "SPAMSSparseGroupL1LogReg"
scheme_used = "SPAMSSparseGroupL1L2LogReg"
# a tag for the precise version of the estimator used as the alg
name_of_the_estimator_used = "SPAMSCustomEstimator0"
# the type of model produced (omc, allfts, mutlitask, etc.)
# the_model_compared = "Allfts_Multitaskof6"
the_model_compared = "CommonFtsMT3ofTop4FSsL1"
# Name of the trial that will be run for this exact analysis
tag_num_trial = "Trial2MT3ofTop4FSsL130seeds" # = "Trial_test" for testing
# name of the base dir where to put our results files
basedir = "/home/amad/PALADIN_1/3CEREBRO/garage/projects/ATIP3/ATIP3_ML/ATIP3_ML_dev_version/data_acquisition/1_pipelines_or_tasks/Multitask_learning/res_learning_tasks/outputs/running/"
...
if tag_decision_make_log == "yes":
	...
	sys.stdout = open(basedir + 'Output_' + task_type + "_" + scheme_used + "_" + the_model_compared + "_" + tag_ctype + "_" + tag_drugname + "_" + tag_profilename + "_" + tag_num_trial + ".o", 'w')
...
# param_grid_gallery = param_grid_lambdas123_space_maker1("geom",0.0001,100.0,200,"yes",None,"yes") # 200 values of lambda1 in sklearn (C) ##! check the key name of the param (move it also)
...
for a_seed in list_seeds:  # test a_seed = 0, # a_seed = 1
	...
	modelselector_by_GSCV = model_selection.GridSearchCV(SPAMSCustomEstimator0(groups=groups_in_data), param_grid, scoring=grid_scorer, cv=10, n_jobs=-1) # same as before but without stating the refit # cv option gives stratification for classifiers/classifier mixin
	# - fit lasso
