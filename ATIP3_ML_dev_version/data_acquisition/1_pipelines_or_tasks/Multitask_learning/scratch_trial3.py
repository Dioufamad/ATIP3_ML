# a copycat of the C1_gridsearchcv_sklearn_corr1_MCC script but using the cohen kappa score as a metric (ie our redundancy as top notch metric)
...
class CustomSPAMSestimatorLossIsSQUARERegulIsSPARSEGROUPLASSOL2V1(base.BaseEstimator, base.ClassifierMixin):  # LossIsSQUARERegulIsSPARSEGROUPLASSOL2V1 would not be in full name
    """
...
for a_seed in list_seeds:  # test a_seed = 0
    ...
    # - the scoring metrics
    cks_as_scorer = make_scorer(cohen_kappa_score) # cks scoring line beta 2
    # - put the model in gridsearchcv (model is CustomSPAMSestimatorLossIsSQUARERegulIsSPARSEGROUPLASSOL2V1(groups=groups_in_data))
    model_gs = model_selection.GridSearchCV(CustomSPAMSestimatorLossIsSQUARERegulIsSPARSEGROUPLASSOL2V1(groups=groups_in_data), param_grid, scoring=cks_as_scorer, cv=10, n_jobs=-1)  # same as before but without stating the refit (beta 2)
    # - fit lasso
    ...
    model_retained_asbest_testscore = cohen_kappa_score(y_test, model_retained_asbest.predict(X_test))
    single_task_lasso_test_scores.append(model_retained_asbest_testscore)
    print("- Finished working with seed", a_seed)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~trying out spams bespoke1
